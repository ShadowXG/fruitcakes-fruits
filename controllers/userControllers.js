...
// POST -> /users/login
// This route creates a new session in our db(and in the browser)
    ...
    User.findOne({ username })
        .then(async (user) => {
            ...
            if (user) {
                ...
                // user.password -> is saved in the db
                ...
                if (result) {
                    ...
                    // we'll send a 201 respons and the user as json(for now)
                    // we'll update this after a couple test to adhere to best practices
                    // res.status(201).json({ username: user.username })
                    res.redirect('/')
                } else {
